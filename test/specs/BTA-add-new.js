...
describe('BTA page - Requester', () => {
    ...
    it('should save a ticket successfully (add 1 user)"', () => {
        ...
        coreHelper.inputText(BTAAddNewPage.SEARCH_ADD_USER_DIALOG,credentials.HQ_G1toG4.Employee_1.username);
        browser.pause(1000);
...
describe('BTA page - First_Approval', () => {
    const credentials = coreHelper.readFile('../utils/BTAdata.json');
    
    it('should navigate to Add BTA page successfully',() => {
        LoginPage._open();
        LoginPage._loginByForms(credentials.HQ_G1toG4.First_Approval.username, credentials.HQ_G1toG4.First_Approval.password);
...
describe('BTA page - Second_Approval', () => {
    const credentials = coreHelper.readFile('../utils/BTAdata.json');
...
describe('BTA page - Admin_Checker', () => {
    const credentials = coreHelper.readFile('../utils/BTAdata.json');
    
    it('should navigate to Add BTA page successfully',() => {
        LoginPage._open();
        LoginPage._loginByForms(credentials.HQ_G1toG4.Admin_Checker.username, credentials.HQ_G1toG4.Admin_Checker.password);
        // coreHelper.scrollToFooter();
        coreHelper.clickAndWait(DashBoard.BTA_BUTTON,BTAAddNewPage.TITLE);
        expect(BTAAddNewPage.TITLE).toBeExisting();
    });
...
describe('BTA page - Admin_Manager', () => {
    const credentials = coreHelper.readFile('../utils/BTAdata.json');
    
    it('should navigate to Add BTA page successfully',() => {
        LoginPage._open();
        LoginPage._loginByForms(credentials.HQ_G1toG4.Admin_Manager.username, credentials.HQ_G1toG4.Admin_Manager.password);
        // coreHelper.scrollToFooter();
        coreHelper.clickAndWait(DashBoard.BTA_BUTTON,BTAAddNewPage.TITLE);
        expect(BTAAddNewPage.TITLE).toBeExisting();
    });
